//
// Inpired by https://github.com/dog-qiuqiu/Android_NCNN_yolov4-tiny
//
...
Yolo::Yolo(AAssetManager* mgr, const char *param, const char *bin) {
    Net = new ncnn::Net();
...
std::vector<BoxInfo> Yolo::detect(JNIEnv* env, jobject image) {
...
std::vector<BoxInfo>
Yolo::decode_infer(ncnn::Mat &data,  const cv::Size &frame_size) {
    std::vector<BoxInfo> result;
